...
/** Диалоговый ввод одной строкой (для запуска без флагов). */
...
/**
 * Если пользователь передал относительный путь (с / или \ и существуют такие сегменты) — используем его.
 * Иначе — ищем по имени файла по всему репозиторию с умным приоритетом:
 *  1) пути, содержащие "app/src/main/java" или "app/src/main/kotlin"
 *  2) пути в любых "src/main/java|kotlin"
 *  3) короче относительный путь (меньше папок)
 *  4) .kt предпочтительнее .java
 */
